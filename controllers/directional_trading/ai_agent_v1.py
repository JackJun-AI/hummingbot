...
class AIAgentV1Controller(DirectionalTradingControllerBase):
    ...
    async def update_processed_data(self):
        ...
        try:
            self.logger().info("=" * 60)
            self.logger().info("🤖 Starting AI decision cycle...")
            ...
            self.logger().info(f"✅ Decision cycle completed - {len(decisions)} actions generated")
            self.logger().info("=" * 60)
...
"""
    ...
    def _parse_ai_response(self, response: str) -> List[Dict]:
        ...
        try:
            ...
            self.logger().info(f"Parsed {len(decisions)} decisions from AI")
            return decisions
    ...
    def _validate_decisions(self, decisions: List[Dict], context: Dict) -> List[Dict]:
        ...
        for decision in decisions:
            ...
            if not action or not symbol:
                self.logger().warning(f"Invalid decision: missing action or symbol - {decision}")
                continue
            
            # 检查交易对是否在配置中
            if symbol not in self.config.trading_pairs:
                self.logger().warning(f"Symbol {symbol} not in configured pairs")
                continue
            
            # 检查仓位数量限制
            if action in ["open_long", "open_short"]:
                if current_positions >= self.config.max_concurrent_positions:
                    self.logger().warning(f"Max positions reached, skipping {action} for {symbol}")
                    continue
                ...
                if take_profit_pct < stop_loss_pct * 1.5:
                    self.logger().warning(f"Risk/Reward ratio too low for {symbol}, adjusting")
    ...
    def determine_executor_actions(self) -> List[ExecutorAction]:
        ...
        for decision in ai_decisions:
            ...
            try:
                ...
                self.logger().error(f"Error creating action for {symbol}: {e}")
